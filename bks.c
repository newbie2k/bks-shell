...
char *getString(void) {
	...
	word = malloc(sizeof(char) * 1);  // Assinged by malloc because word is
					  // going to used by realloc function.

	while ((c = getchar()) != ' ') {
		if (c == '\n') {
			STATUS = NEWLINE;  // Used to Know how getString()
					   // function return by pressing ENTER
					   // KEY or SPACE KEY
			break;
		}
...
int main(void) {
	...
	do {
		...
		cmd = getString();
		if (STATUS !=
		    NEWLINE) {  // If ENTER KEY is not press i.e. we press SPACE
				// KEY and about to give Options or Argument.
			tmp = getString();
		...
		printf("Command = %s, Argument = %s and Option = %s ", cmd, opt,
		       arg);

	} while ((strcmp(cmd, "quit") != 0) &&
		 (strcmp(cmd, "exit") != 0));  // Type "quit" or "exit" (without
					       // quots) to exit from program.
}
